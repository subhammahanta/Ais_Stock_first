...
@Repository
public interface RepairReplaceDeviceMasterRepository extends JpaRepository<RepairReplaceDeviceMaster, Long> {

	default Page<RepairReplaceDeviceMaster> findAll(GenericRequestBody requestDTO, Pageable pageRequest) {
		try {
	...
	Page<RepairReplaceDeviceMaster> findAll(Specification<RepairReplaceDeviceMaster> search, Pageable pageable);

	static Specification<RepairReplaceDeviceMaster> search(GenericRequestBody entity) {
		return (root, cq, cb) -> {
	...
	@Query(value = "SELECT * FROM repair_replace_device_master r WHERE r.eway_bill_no =?1", nativeQuery = true)
	List<RepairReplaceDeviceMaster> findByEwayBill(String ewayBillNo);
