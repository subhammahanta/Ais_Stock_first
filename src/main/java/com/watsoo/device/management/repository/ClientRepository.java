...
public interface ClientRepository extends JpaRepository<Client, Long> {
	...
	default Page<Client> findAll(ClientDTO clientDTO, Pageable pageRequest) {
		try {
	...
	Page<Client> findAll(Specification<Client> searchAll, Pageable pageRequest);

	static Specification<Client> searchAll(ClientDTO clientDTO) {
		return (root, cq, cb) -> {
			...
			if (clientDTO.getCompanyName() != null && clientDTO.getCompanyName() != "") {
				//p.getExpressions().add(cb.equal(root.get("companyName"), clientDTO.getCompanyName()));
				p.getExpressions().add(cb.like(root.get("companyName"), "%" + clientDTO.getCompanyName() + "%"));
				isNotPresent = false;
			}
	...
	Client findByCompanyNameAndEmailAndPanNumber(String companyName, String email ,String panNumber);
	...
	Client findByCompanyNameAndEmail(String companyName, String email);
	
	@Query(value="SELECT * FROM client WHERE company_name =:companyName OR email =:email OR pan_number =:panNumber",nativeQuery = true)
	Client findByCompanyNameAndEmailAndPanNumberV2(String companyName, String email ,String panNumber);
