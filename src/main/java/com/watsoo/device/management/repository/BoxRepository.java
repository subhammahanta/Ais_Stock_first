...
public interface BoxRepository extends JpaRepository<Box, Long> {
	...
	default Page<Box> findAll(BoxDTO boxDto, Pageable pageRequest) {
		try {
	...
	Page<Box> findAll(Specification<Box> searchAll, Pageable pageRequest);

	static Specification<Box> searchAll(BoxDTO boxDto) {
		return (root, cq, cb) -> {
			...
			if (boxDto.getStateId() != null) {
				p.getExpressions().add(cb.equal(root.get("state").get("id"), boxDto.getStateId()));
				isNotPresent = false;
			}
	...
	List<Box> findDevicePackedBoxes(String status);
	...
	List<Box> findByIssueId(Long issuedId);

	@Query(value = "select * from box b where b.id IN(:boxIds) and b.issue_id is not null", nativeQuery = true)
	List<Box> findByIdInAndIssuedList_idNotNull(@Param("boxIds") List<Long> boxIds);
	...
	List<Box> findBoxesByBoxNo(List<String> boxIds, String status);
...
//	@Transactional
	...
	@Query(value = "select * from box b where b.box_no =:boxCode AND b.status =:status", nativeQuery = true)
	Box findByBoxCode(String boxCode, String status);

	@Query(value = "select * from box b where b.box_no IN (:boxCode) AND b.status =:status", nativeQuery = true)
	List<Box> findByBoxCodes(List<String> boxCode, String status);

	@Query(value = "select * from box b where b.box_no =:boxCode", nativeQuery = true)
	Box findByBoxNumber(String boxCode);
